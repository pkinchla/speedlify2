...
/**
 * Starter projects and templates, pulled out of the community list.
 *
 * They are legitimately in 11ty/11ty-community, so deleting them from the
 * generated file would only last until the next import. Excluding them here
 * instead keeps that file exactly as upstream sends it, and keeps this
 * categorisation from being undone by a re-import.
 *
 * Written in normalized form, which is how the importer stores them.
 */
const STARTERS = ["https://11skeleton.vercel.app/"];
...
export default {
	...
	groups: {
		...
		"11ty-community": {
			...
			// Starters are filtered out rather than deleted upstream; see STARTERS.
			sites: community.urls.filter((url) => !STARTERS.includes(url)).map((url) => ({ url })),
		},
