 ...
 */
async function emulateDevice(page, formFactor) {
 ...
 */
async function captureWithoutJavaScript(browser, url, formFactor, timeoutMs) {
	let page;

	try {
		page = await browser.newPage();
		await emulateDevice(page, formFactor);
 ...
 */
export async function runAxe(url, { port, timeoutMs = 60000, screenshots = false, formFactor = "mobile" } = {}) {
	let browser;
	let page;
	...
	try {
		...
		if (screenshots) {
			...
			const noJs = await captureWithoutJavaScript(browser, url, page.viewport(), timeoutMs);
			// How much of what you see needed a script to get there.
			shots = { js, noJs, difference: await visualDifference(browser, js, noJs) };
		}
