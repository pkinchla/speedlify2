 ...
 */
<<<<<<< HEAD
export async function runAxe(url, { port, timeoutMs = 60000, userAgent = null } = {}) {
=======
...
<<<<<<< HEAD
		// A WAF rule on the site's end can skip its bot check for a tagged
		// User-Agent. Lighthouse's own navigation gets through untagged — only
		// this separate page's request needs it. See `site.userAgent` in
		// config/sites.js.
...
>>>>>>> 7c6fe24bea72fcb798d9986c545d53d54cc25777
		...
		if (screenshots) {
			...
			shots = { js, noJs, difference: noJs ? await visualDifference(browser, js, noJs) : null };
		}
