...
export async function loadConfig(file = "config/sites.js") {
	...
	/**
	 * URLs whose screenshot has to come from our own measurement.
	 *
	 * The screenshot service is an automated client like any other, so a site
	 * behind a bot check serves it the challenge page — and the site page then
	 * shows a picture of Cloudflare's waiting room under that site's name. Our
	 * own Chrome often gets through where the service does not, so for these we
	 * keep Lighthouse's full-page capture and use that instead.
	 */
	const screenshotBlocked = new Set((config.screenshotBlocked || []).map(normalizeUrl));
	...
	for (let [groupId, group] of Object.entries(config.groups)) {
		...
		for (let site of group.sites || []) {
			...
			byUrl.set(url, {
				...
				// Capture our own screenshot for this one, and prefer it on the page.
				screenshotBlocked: screenshotBlocked.has(url) || site.screenshotBlocked === true,
