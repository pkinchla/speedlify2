 ...
 * magnitude more than the rest of a measurement put together.
 *
 ...
 */
/**
 * The whole scrollable page as one image, from Lighthouse's own capture.
 *
 * Only taken for sites the screenshot service cannot reach — see
 * `screenshotBlocked` in config/sites.js. It is an order of magnitude larger
 * than a filmstrip frame (237 KB and 16,000 pixels tall on one site here), and
 * every byte is committed, so it is not something to collect by default.
 *
 * Full page rather than the viewport: the header thumbnail crops to the top
 * anyway, and having the rest means a future page can show more without another
 * measurement.
 */
export function extractFullPageScreenshot(lhr) {
	const shot = lhr?.fullPageScreenshot?.screenshot;
	// Not jpeg: Lighthouse 13 encodes this one as WebP, unlike the filmstrip
	// frames beside it. The type is read rather than assumed, so the file lands
	// on disk with an extension that matches its bytes and the next re-encoding
	// upstream does not silently produce mislabelled images.
	const match = /^data:image\/(\w+);base64,(.+)$/.exec(shot?.data ?? "");
