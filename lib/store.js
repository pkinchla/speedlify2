...
export class ResultStore {
	...
	write(record) {
		...
		const { screenshots, fullPage, ...stored } = record;
		this.writeFilmstrip(dir, screenshots, stored.timestamp);
		this.writeFullPage(dir, fullPage);
	...
	/**
	 * The whole-page screenshot, for sites the screenshot service cannot reach.
	 *
	 * One file per site, replaced each measurement — a history of screenshots is
	 * a much larger product than a history of numbers, and this image is already
	 * the biggest thing a measurement produces.
	 *
	 * A null leaves whatever is there alone. A run that failed before rendering
	 * has no opinion about what the site looks like, and the last good picture
	 * beats none.
	 */
	writeFullPage(dir, shot) {
		if (!shot?.buffer?.length) return null;

		fs.mkdirSync(dir, { recursive: true });

		// Named for what it is. The manifest carries the filename so the report
		// does not have to guess the extension, and so a change of format
		// upstream does not leave a stale file of the old type behind.
		const file = `screenshot.${shot.type === "jpeg" ? "jpg" : shot.type}`;
		for (let stale of fs.readdirSync(dir)) {
			if (/^screenshot\.(jpg|jpeg|png|webp|avif)$/.test(stale) && stale !== file) {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ file, width: shot.width, height: shot.height }, null, 2) + "\n",
		);
		return file;
	}
