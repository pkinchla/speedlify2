...
export class Runner {
	...
	async #axe(url, site) {
		...
		const result = await runAxe(url, {
			...
			// Same opt-out as the filmstrip: a category that asked for no pictures
			// gets none, including these.
			screenshots: (site?.screenshots ?? "filmstrip") !== "none",
		});
