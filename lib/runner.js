...
export class Runner {
	 ...
	 *
	 * A site with `userAgent` set (see config/sites.js) runs this in its own
	 * throwaway Chrome instead of the one Lighthouse just navigated in — see
	 * #axeWithOwnChrome.
	 *
	 * Never fatal.
	 */
	async #axe(url, site, median) {
		...
		const options = {
			...
			// pictures.
			noJsScreenshot: (site?.screenshots ?? "filmstrip") !== "none",
		};

		const result = site.userAgent
			? await this.#axeWithOwnChrome(url, site.userAgent, options)
			: await runAxe(url, { port: (await this.#ensureChrome()).port, ...options });
	 ...
	 */
	async #axeWithOwnChrome(url, userAgent, options) {
		let chrome;
		try {
			chrome = await chromeLauncher.launch({ chromeFlags: CHROME_FLAGS });
			return await runAxe(url, { port: chrome.port, userAgent, ...options });
		} finally {
			try {
