...
import { extractMetrics, extractScreenshots, extractFullPageScreenshot, medianRun } from "./metrics.js";
...
export class Runner {
	...
	async #runOnce(url) {
		...
		return {
			...
			fullPage: extractFullPageScreenshot(result.lhr),
		};
	}
	...
	async measure(site) {
		...
		for (let i = 0; i < this.runs; i++) {
			try {
				const { metrics, screenshots, fullPage } = await this.#runOnce(url);
		...
		const axe = attempts.length ? await this.#axe(url, site.userAgent) : null;
		...
		return {
			...
			// Same run as the filmstrip and the numbers, and only where the
			// screenshot service is blocked — see ResultStore.write, which puts it
			// on disk and keeps it out of the record.
			fullPage: site.screenshotBlocked ? fullPages[attempts.indexOf(median)] : null,
	...
	async #axe(url, userAgent) {
		if (!this.axeEnabled) return null;

		const result = userAgent
			? await this.#axeWithOwnChrome(url, userAgent)
			: await runAxe(url, { port: (await this.#ensureChrome()).port, timeoutMs: this.timeoutMs });
	...
	/**
	 * Run axe in a fresh, throwaway Chrome of its own, instead of the shared
	 * one Lighthouse just navigated in.
	 *
	 * A tagged User-Agent alone is not enough here: Cloudflare (and similar bot
	 * management) can flag the whole browser session the first time it sees
	 * Lighthouse's own untagged, automated navigation, and a *new page* in that
	 * same Chrome inherits the flag even with the right header — verified live
	 * against paulkinchla.com on 2026-08-25, a fresh Chrome process was the only
	 * thing that got a tagged request through. Costs one extra Chrome launch,
	 * paid only by sites that set `userAgent`.
	 */
	async #axeWithOwnChrome(url, userAgent) {
		let chrome;
		try {
			chrome = await chromeLauncher.launch({ chromeFlags: CHROME_FLAGS });
			return await runAxe(url, { port: chrome.port, timeoutMs: this.timeoutMs, userAgent });
		} finally {
			try {
