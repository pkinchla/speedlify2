...
export function selectBatch(sites, store, options = {}) {
	...
	const candidates = pool.map((site) => {
		...
		return { site, lastAt, ageHours, failing, consecutiveFailures, retired };
	});

	const eligible = [];
	const skipped = { fresh: 0, backoff: 0, retired: 0 };
	...
	// Never-measured first (Infinity sorts to the front), then oldest first.
	eligible.sort((a, b) => {
