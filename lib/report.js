...
function buildEntry(site, store, config, now, aliases, context) {
	...
	const entry = {
		...
		filmstrip,
		// Preferred over the screenshot service where it exists — see
		// `screenshotBlocked` in config/sites.js.
		ownScreenshot,
 ...
 */
/**
 * Our own full-page screenshot, for a site the screenshot service cannot reach.
 *
 * Only written for URLs listed in `screenshotBlocked`, so its absence is the
 * ordinary case rather than a fault — the page falls back to the service.
 */
function readOwnScreenshot(resultsDir, hash) {
	...
	// The manifest names the file, because the format is Lighthouse's choice
	// rather than ours — WebP today, and the extension has to follow it.
	if (!manifest.file || !fs.existsSync(path.join(resultsDir, hash, manifest.file))) return null;

	return {
		src: `/results/${hash}/${manifest.file}`,
