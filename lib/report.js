...
function siteOfTheDay(entries, now) {
	const featured = entries.filter(isPerfectFeatured);
	...
	for (let entry of featured) {
		// djb2 over the pair, hex-padded so the comparison below is lexicographic
		// on a fixed width rather than numeric on a float.
		let h = 5381;
	...
	return best ? { date: day, entry: best, pool: featured.length } : null;
}
 ...
 */
/**
 * Byte sizes a person would choose for an axis, in order.
 *
 * The buckets are cut from a window of this ladder rather than from arithmetic
 * on the data, so the edges stay round — "250 kB" rather than "263.4 kB" — and
 * two categories whose data happens to be similar get identical axes instead of
 * two nearly-identical ones.
 */
const BYTE_LADDER = [
	1e3, 2.5e3, 5e3, 10e3, 25e3, 50e3, 100e3, 250e3, 500e3,
	1e6, 2.5e6, 5e6, 10e6, 25e6, 50e6, 100e6,
];

/** Compact axis label: 10k, 250k, 1M. */
function ladderTick(bytes) {
	if (bytes >= 1e6) return `${Math.round((bytes / 1e6) * 10) / 10}M`;
	return `${Math.round(bytes / 1e3)}k`;
}

/** Human bucket label from a pair of edges. */
function ladderLabel(floor, max) {
	const size = (v) => (v >= 1e6 ? `${Math.round((v / 1e6) * 10) / 10} MB` : `${Math.round(v / 1e3)} kB`);
	if (max === Infinity) return `Over ${size(floor)}`;
	if (floor === 0) return `Under ${size(max)}`;
	return `${size(floor)}–${size(max)}`;
}
 ...
 */
function scaleBuckets(values, { zeroBucket = false } = {}) {
	...
	let top = BYTE_LADDER.findIndex((v) => v >= p90);
	if (top === -1) top = BYTE_LADDER.length - 1;
	const first = Math.max(0, Math.min(top - rungs + 1, BYTE_LADDER.length - rungs));
	const edges = BYTE_LADDER.slice(first, first + rungs);
	...
	for (let edge of edges) {
		buckets.push({ max: edge, label: ladderLabel(zeroBucket && floor === 1 ? 0 : floor, edge), tick: ladderTick(edge) });
		floor = edge;
	}
	buckets.push({ max: Infinity, label: ladderLabel(floor, Infinity), tick: "+" });
