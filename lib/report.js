...
/**
 * Script bytes, on their own scale.
 *
 * Not the page-weight edges: JavaScript is an order of magnitude smaller than
 * the page it sits in — the middle site here ships 12 kB of it against 256 kB
 * of everything — and reusing those buckets put three quarters of the corpus in
 * the first bar.
 *
 * "None" is its own bucket rather than the bottom of the first. 298 sites here
 * ship no JavaScript at all, and on a list this full of static sites that is a
 * fact worth a bar of its own instead of being rounded in with the ones that
 * ship a little.
 */
const SCRIPT_BUCKETS = [
	{ max: 1, label: "No JavaScript", tick: "0" },
	{ max: 10e3, label: "Under 10 kB", tick: "10k" },
	{ max: 50e3, label: "10–50 kB", tick: "50k" },
	{ max: 100e3, label: "50–100 kB", tick: "100k" },
	{ max: 250e3, label: "100–250 kB", tick: "250k" },
	{ max: 500e3, label: "250–500 kB", tick: "500k" },
	{ max: Infinity, label: "Over 500 kB", tick: "+" },
];
...
function buildStats(entries, { featuredOnly = false } = {}) {
	...
	return {
		...
		recentlyMeasured: measured
			...
			.map((e) => ({ displayUrl: e.displayUrl, slug: e.slug, timestamp: e.latest.timestamp })),
		weightBuckets: bucketize(weights, WEIGHT_BUCKETS),
		...
		scriptBuckets: bucketize(scripts, SCRIPT_BUCKETS),
