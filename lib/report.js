...
export async function buildReport({
	...
	// Retired sites leave the dataset here, before anything ranks or groups them.
	// They are not slow sites or failing sites — they are not sites, and a
	// parking page in a leaderboard is worse than a missing row. Reported
	// separately below so the drop is visible rather than silent.
	const retired = all.filter((e) => e.retiredTo);
	const entries = all.filter((e) => !e.retiredTo);
	...
	return {
		...
		// One record per legacy API filename, flat and self-contained.
		//
		...
		compatRoutes: entries.flatMap((e) =>
			e.compatHashes.map((hash) => ({
				hash,
				lighthouse: e.latest?.lab?.scores
					? {
							performance: e.latest.lab.scores.performance ?? null,
							accessibility: e.latest.lab.scores.accessibility ?? null,
							bestPractices: e.latest.lab.scores["best-practices"] ?? null,
							seo: e.latest.lab.scores.seo ?? null,
						}
					: null,
			})),
		),
 ...
 */
/**
 * The legacy API filenames one URL can be known by.
 *
 * Deduped, with the stored form first so `shortHash` — the primary — stays the
 * first thing emitted.
 */
function compatHashes(url) {
	...
	return [...new Set([...forms].map(shortHash))];
}
...
function buildEntry(site, store, config, now, aliases, context) {
	...
	const entry = {
		...
		compatHashes: compatHashes(site.url),
...
function buildStats(entries) {
	...
	return {
		...
		// actual visits.
		perfect: measured.filter(isPerfectFeatured).length,
		...
		perfectEleventy: measured.filter(
			(e) => isPerfectFeatured(e) && ELEVENTY_IDS.has(e.generator?.id ?? e.presumedGenerator?.id),
		).length,
