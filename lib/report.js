...
/**
 * The middle value of each of the six rings, across everything measured.
 *
...
function medianRings(measured) {
	 ...
	 *
	 * Taking the lower keeps the figure to something actually observed, and
	 * errs toward the worse of the two — which is the right direction for a
	 * summary of a fleet.
	 */
	const median = (values) => {
	...
	return {
		scores: Object.fromEntries(Object.entries(scores).map(([key, values]) => [key, median(values)])),
		axe: median(axe),
		cwvFailures: median(cwv),
...
function buildStats(entries, { featuredOnly = false } = {}) {
	...
	return {
		...
		// The middle value of each ring across the fleet, computed independently.
		medians: medianRings(measured),
